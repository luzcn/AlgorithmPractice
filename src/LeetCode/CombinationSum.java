 ...
 * number may be chosen from C unlimited number of times. All numbers (including
...
public class CombinationSum
{
	...
	public ArrayList<ArrayList<Integer>> combinationSum(int[] arr,
			int target)
	{
	...
	public ArrayList<ArrayList<Integer>> getCombSum(int[] arr,
			int target, int sum, int index, ArrayList<Integer> currentCandidates,
	 ...
	 * (including target) will be positive integers Elements in a combination
	 * (a1, a2,.., ak) must be in non-descending order The solution set must not
	 * contain duplicate combinations. For example, given candidate set
	 * 10,1,2,7,6,1,5 and target 8, A solution set is: [1, 7][1, 2, 5][2, 6][1,
	 * 1, 6]
	 * 
	 * @param candicates
	 * @param target
	 * @return
	 */
	public ArrayList<ArrayList<Integer>> combinationSum2(int[] arr,
			int target)
	{
		ArrayList<ArrayList<Integer>> solution = new ArrayList<ArrayList<Integer>>();
		
		for (int i = 0;i<arr.length;i++)
			getCombSum2(arr, target, 0, i, new ArrayList<Integer>(),
					solution);
		
		return solution;
	}

	private ArrayList<ArrayList<Integer>> getCombSum2(int[] arr,
			int target, int sum, int index,
			ArrayList<Integer> currentCandidates,
			ArrayList<ArrayList<Integer>> solution)
	{
		...
		else
		{
			...
			{
				getCombSum2(arr,target,sum+arr[index],i,currentCandidates,solution);
				currentCandidates.remove(currentCandidates.size()-1);
			}
		}
		return solution;
	...
	public static void main(String[] args)
	{
		...
		ArrayList<ArrayList<Integer>> solution = cs.combinationSum2(nums, 8);
