...
public class InsertInterval
{
	 ...
	 * two intervals.
	 * 
	 * Therefore, our solution goes as following: 
	 * 1) Go through the list of intervals, 
	 * find out the ones that are not overlapped to the new interval, put them into a new list 
	 * 
	 * 2) Check if there are overlapped intervals 
	 * 2.1) if true, then merger the overlapped intervals and the new interval into one 
	 * 2.2) if false, then add the new interval into the list.
	 * 
	 * @param intervals
	 * @param newInterval
	 * @return
			...
			ArrayList<Interval> intervals, Interval newInterval)
	{
		...
		for (Interval I:intervals)
		{
			...
			{
				int start = I.start>newInterval.start?newInterval.start:I.start;
	...
	public static boolean isOverLap(Interval l1, Interval l2)
	{
		...
		if (l1.start>l2.start && l1.start<l2.end)
			return true;
		if ((l1.start>l2.start && l1.end<l2.end) || (l2.start>l1.start && l2.end<l1.end))
			return true;
